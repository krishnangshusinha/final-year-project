...
const ElectricityInstallPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchInstallRequests = async () => {
            ...
            try{
                const {data} = await axios.get("http://localhost:3001/admin/electricity/installs");
    ...
    const handleStatus = async (id , status) => {
        try {
            const {data} = await axios.put(`http://localhost:3001/admin/electricity/install/${id}/${status}`)
