...
const GarbageRepairPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchUniqueIDs = async () => {
            ...
            try{
                const {data} = await axios.get("http://localhost:3001/admin/garbage/repairs");
    ...
    const deleteComplaint = async (id) => {
        ...
        try{
            const {data} = await axios.delete(`http://localhost:3001/admin/delete/garbage/repairs/${id}`)
    ...
    const handleStatus = async (id , status) => {
        try {
            const {data} = await axios.put(`http://localhost:3001/admin/garbage/repair/${id}/${status}`)
